...
class LoansFromEtmp @Inject() extends Transformer {
  ...
  def transform(loans: EtmpLoans): Loans =
    Loans(
      ...
      optSchemeHadLoans = loans.schemeHadLoans.map(fromYesNo),
        ...
        .map { loanTransaction =>
          ...
          LoanTransactions(
            ...
            loanAmountDetails = LoanAmountDetails(
              ...
              optCapRepaymentCY = loanTransaction.capRepaymentCY,
              optAmountOutstanding = loanTransaction.amountOutstanding
            ),
            ...
            optArrearsPrevYears = loanTransaction.arrearsPrevYears.map(fromYesNo),
