...
class PsrDeclarationFromEtmpSpec extends EtmpTransformerSpec {
  ...
  private val transformation: PsrDeclarationFromEtmp = new PsrDeclarationFromEtmp()

  "PsrDeclarationFromEtmp" should {
    "transform correctly for a PSA" in {
      val declaration = EtmpPsrDeclaration(
      ...
      val expected: PsrDeclaration = PsrDeclaration(
        submittedBy = SubmitterType.PSA,
        submitterId = "submitterId",
      ...
      transformation.transform(declaration) shouldMatchTo expected
    }
